...
test_image_save_dir_label = options.params.test_image_save_dir_label
test_image_save_dir_pred = options.params.test_image_save_dir_pred
...
def read_images(seg_dir, lab_dir):
    ...
    return seg_images, lab_images, lab_names
...
def test():
    ...
    # follow this routine.
    seg_images, lab_images, lab_names = read_images(test_seg_dir, test_lab_dir)
    ...
    with tf.Graph().as_default() as g:
        ...
        inp_batch = tf.placeholder(dtype = tf.float32, shape = [None, image_H, image_W, nChann])
...
#           model_clear = chop_forward(seg_images[img_index], sess, inp_batch, pred_model)
            ...
            if test_save_image:
                ...
                image_path = os.path.join(test_image_save_dir_label, lab_names[img_index])
                test_label_img = lab_image[0, :, :, 0]
                misc.imsave(image_path, test_label_img*norm)
                
                
                image_path = os.path.join(test_image_save_dir_pred, lab_names[img_index])
            ...
            with open(test_log_dir + "test_label_records.txt", "a") as file:
                
                format_str = "%s\t%d\t%.4f\n"
                file.write(format_str % (lab_names[img_index].split(".")[0], img_index, mo_dics))
        ...
        print('  Processing Image %d/%d...' % (img_index + 1, num_images))
        print('  model_loss = %.6f\n' % (model_loss / num_images))
        print('  model_psnr = %.6f\n' % (model_psnr / num_images))
        print('  model_ssim = %.6f\n' % (model_ssim / num_images))
        print('  model_dics = %.6f\n' % (model_dics / num_images))
