...
#    space_mask = tf.ones([batch_size, inpRow, inpCol, 1], dtype = tf.float32, name = 'space_mask')
    ...
    num_names = len(train_seg_images)
    ...
    for i in range(epoch):
        ...
        for k in index:
            ...
            inpBatch, labBatch = get_train_batch(train_seg_images[k:k + batch_size], train_lab_images[k:k + batch_size])
            ...
            if (step + 1) % train_log_freq == 0:
                ...
                with open(valid_save_dir + "train_records.txt", "a") as file:
                    format_str = "%d\t%.6f\t%.6f\t%.6f\t%.6f\n"
                    file.write(str(format_str) % (step + 1, model_loss, model_psnr, model_ssim, model_dics))
            ...
            if (step == 0) or ((step + 1) % 500 == 0) or ((step + 1) == max_steps):
               ...
               # mean_wt, mean_et, mean_tc = calc_WT_ET_and_TC(valid_label_batch, valid_model, valid_batch_size)
                ...
                if valid_save_images:
                    ...
                    image_path = valid_image_save_dir + str(step + 1) + '_label.png'
                    valid_label_img = valid_label_batch[index_valid, :, :, 0]
                    misc.imsave(image_path, valid_label_img*norm)
                        
                    image_path = valid_image_save_dir + str(step + 1) + '_model.png'
